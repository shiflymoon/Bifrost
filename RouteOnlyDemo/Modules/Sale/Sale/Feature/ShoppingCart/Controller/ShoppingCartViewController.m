...
- (void)reloadViewModel {
    ...
    //goods info section
    {
        ...
        for (ShoppingCartItem *item in [ShoppingCartManager sharedInstance].cartItemDict.allValues) {
            ...
            cellModel.height = 44;
            NSDictionary *paramDic = @{kRouteGoodsModelParamKeyGoodsId:item.goodsId};
            
            NSDictionary * goods = [Bifrost handleURL:kRouteGoodsServiceGoodsById complexParams:paramDic completion:nil];
            YZWeak(item)
            cellModel.renderBlock = ^UITableViewCell * _Nonnull(NSIndexPath * _Nonnull indexPath, UITableView * _Nonnull tableView) {
                ...
                NSString *text = BFStr(@"%@ : ￥%.2f x %lu", goods[kRouteGoodsModelParamKeyGoodsName], [goods[kRouteGoodsModelParamKeyGoodsPrice] floatValue], item.num);
...
- (CGFloat)totalPrice {
    ...
    for (ShoppingCartItem *item in self.shoppingCartItemList) {
        NSDictionary *paramDic = @{kRouteGoodsModelParamKeyGoodsId:item.goodsId};
        NSDictionary * goods = [Bifrost handleURL:kRouteGoodsServiceGoodsById complexParams:paramDic completion:nil];
        totalPrice += [goods[kRouteGoodsModelParamKeyGoodsPrice] floatValue] * item.num;
    }
    return totalPrice;
}
